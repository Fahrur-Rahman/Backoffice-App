...
class BackofficeController extends Controller
{
    ...
    {
        $candidate = DB::table('backoffice.link as dt1')->join('recruitmen.candidate as dt2', 'dt2.candidateid', '=', 'dt1.candidateid')
        ->join('backoffice.schedule','schedule.candidatelistid', '=', 'dt1.candidatelistid')
        ->join('backoffice.interview','interview.interviewid','=','schedule.scheduleid')
        ->leftjoin('backoffice.interviewbyclient','interviewbyclient.interviewclientid','=', 'interview.interviewid')
        // ->join(DB::raw("(SELECT `jml`, CASE WHEN `statusid` = 5 THEN `interviewbyclient`.`statusclientid` ELSE `interview`.`statusid` END AS `anotherstatus` FROM `interview` INNER JOIN `interviewbyclient` ON `interviewbyclient`.`interviewid` = `interview`.`interviewid` INNER JOIN (SELECT MAX(`interviewid`) as `jml` FROM `interview` INNER JOIN `schedule` on `schedule`.`scheduleid` = `interview`.`interviewid` GROUP BY `schedule`.`candidatelistid`) AS `t1` ON `t1`.`jml` = `interview`.`interviewid`) as t2"),function($join){
        //     $join->on("interview.interviewid", "=", "t2.jml");
        // })
        // ->join('backoffice.interviewbyclient','interviewbyclient.interviewclientid','=','interview.interviewid')
        ->orderBy('interviewid', 'DESC')->take(6)
        // ->leftjoin(DB::raw("(SELECT `jml`, CASE WHEN `statusid` = 5 THEN `interviewbyclient`.`statusclientid` ELSE `interview`.`statusid` END AS `anotherstatus` FROM `interview` INNER JOIN `interviewbyclient` ON `interviewbyclient`.`interviewid` = `interview`.`interviewid`
        // INNER JOIN (SELECT MAX(`interviewid`) as `jml` FROM `interview` INNER JOIN `schedule` on `schedule`.`scheduleid` = `interview`.`interviewid` GROUP BY `schedule`.`candidatelistid`) AS `t1` ON `t1`.`jml` = `interview`.`interviewid`) as t2"),function($join){
        //     $join->on("interview.interviewid", "=", "t2.jml");
        // })
        // ->orderBy('interviewid', 'DESC')->take(6)
        ->get();
        // $candi = new Candidate;
        // $candidatebackoffice = DB::table('link')->get();
        // $candidateform =  DB::connection('mysql2')->table('candidate')->join($candidatebackoffice, 'link.candidateid', '=', 'candidate.id')->get();
        // $candidate = join($candidatebackoffice.candidateid,'=', $candidateform.id);
        // $candidate = DB::connection('mysql2')->table('candidate')
        // ->join('candidatelink','candidatelink.candidateid', '=', 'candidate.id')
        // ->join('schedule','schedule.candidatelistid', '=', 'candidatelink.candidatelistidid')
...
// dd($candidate);
        ...
        ->leftjoin(DB::raw("(SELECT `jml`, CASE WHEN `statusid` = 5 THEN `interviewbyclient`.`statusclientid` ELSE `interview`.`statusid` END AS `anotherstatus` FROM `interview` INNER JOIN `interviewbyclient` ON `interviewbyclient`.`interviewclientid` = `interview`.`interviewid` INNER JOIN (SELECT MAX(`interviewid`) as `jml` FROM `interview` INNER JOIN `schedule` on `schedule`.`scheduleid` = `interview`.`interviewid` GROUP BY `schedule`.`candidatelistid`) AS `t1` ON `t1`.`jml` = `interview`.`interviewid`) as t2"),function($join){
            $join->on("interview.interviewid", "=", "t2.jml");
        })
        ...
        $jmlcandidate = schedule::leftJoin('interview','interview.interviewid', '=', 'schedule.scheduleid')
        ->whereNull('interviewid')->count();
